...
import com.ours.constant.Codes;
...
/**
 * created by lyf at 2018/4/20 17:26<br>
 */
...
public class AuthController {
    ...
    @Autowired
    private FmsBizCustInfoService fmsBizCustInfoService;
    ...
    public Json login(@RequestBody String body) {
        ...
        try {
            ...
            if (StringUtils.isEmpty(type)) {
                ...
                int count = fmsBizCustInfoService.selectCount(new EntityWrapper<FmsBizCustInfo>().where("cust_phone={0}", uname));
                if (count == 0) {
                    throw new RuntimeException();
                }
                Subject currentUser = SecurityUtils.getSubject();
                List<SysUser> userList = sysUserService.selectList(new EntityWrapper<SysUser>().eq("uname", uname));
                if (userList != null && userList.size() > 0) {
                    SysUser sysUser = userList.get(0);
                    currentUser.login(new UsernamePasswordToken(sysUser.getUname(), "123456"));
                } else {
                    return Json.fail(oper, "手机号不存在");
                }
                //登录
                //从session取出用户信息
                ...
                return Json.succ(oper)
                        ...
                        .data("nick", json.getString("uname"))
